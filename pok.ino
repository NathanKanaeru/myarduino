/****************************
Jam Waktu Sholat Abadi dengan menggunakan P10 Single Panel Dilengkapi Running Text

Oleh : Arduino Indonesia
Website : www.arduinoindonesia.id
Toko Online : www.workshopelectronics3in1.com
Blog : www.edukasielektronika.com
Profil : www.electronics3in1.co.id
...
// Mendifinisikan Tombol Setting Manual//

#define Jam_bUP A1      // Tombol setting UP ke PIN A1 Arduino
#define tombol_bOK A2   // Tombol setting OK ke PIN A2 Arduino
#define Jam_bDN A3      // Tombol setting DOWN ke PIN A3 Arduino

// Source Code dibawah ini untuk setting RTC agar sesuai dengan Jam pada laptop //
...
setThn:
  ...
  // Setting thn
  if (digitalRead(Jam_bUP) == LOW || val == 'K') // Tombol Setting Tahun
  {                               
  ...
  if (digitalRead(Jam_bDN) == LOW || val == 'L') // Tombol Setting Tahun
  {                               
    delay(10); thn--;                                          // Delay Tombol & Tambah Nilai thn
    if (thn <= 0) 
    {
...
// Interupsi Tombol Menu
  ...
  if (now.second() > 1) 
  {
    ...
    ////Source Code Adzan Sholat Ashar
    ...
    ///Source Code Adzan Sholat Maghrib
    ...
    ///Source Code Adzan Sholat JUMAT
    ...
    ///Source Code Adzan Sholat DZUHUR
    ...
    ///Source Code Adzan Sholat SUBUH
    ...
    if (now.second() > 10) 
    {
      ...
      ///================Pengaturan Koordinat Posisi Untuk Menentukan Posisi Anda, Agar Jam Waktu Sholat Sesuai dan Abadi=================
      double lintang = -8.55555;     //Latitude
      double bujur = 112.16528 ;   //Longitude
      ...
      set_fajr_angle(20.0);           //Sudut Waktu Subuh
      set_isha_angle(18.0);           ////Sudut Waktu Isya
  ...
  if (now.second() > 30) 
  {
    ...
    Teks = weekDay[now.dayOfWeek()] + Konversi(now.date()) + "/" + Konversi(now.month()) + "/" + Konversi(now.year()) + " IMSAK=" + Konversi(hours0) + ":" + Konversi(minutes0) + " SUBUH=" + Konversi(hours1) + ":" + Konversi(minutes1)
           + " DZUHUR=" + Konversi(hours2) + ":" + Konversi(minutes2) + " ASHAR=" + Konversi(hours3) + ":" + Konversi(minutes3)
           + " MAGHRIB=" + Konversi(hours4) + ":" + Konversi(minutes4) + " ISYA'=" + Konversi(hours6) + ":" + Konversi(minutes6) + " | WORKSHOP ELECTRONICS 3 IN 1 | 085790269618 | ";

    int kecepatan;
    kecepatan = 150;     //kecepatan runing teks
    ...
    while (!ret) 
    {
      ...
      {
        ret = dmd.stepMarquee(-2 ,0); ///dianimasikan baris 8 sampai 15, sampai kolom 0
        timer = millis();
        ///TOMBOL SET JAM//
